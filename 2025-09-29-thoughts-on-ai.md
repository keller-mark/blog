...
- I don't like that dedication of so much interest/human-energy to LLMs/chatbots comes at the cost of not pursuing other interesting projects (e.g., non-LLM-based CS/machine learning/bioinformatics).
  - It is unnerving that in academia, funding agencies (and their study sections which are supposed to be panels of experts on specific topics that guide grant funding and decide how tax dollars are spent) seem to have bought into hype around AI just as much as everyone else. (There are of course exceptions, and there are plenty of people doing great research on AI. For instance, I particularly like the [Ai2 Paper Finder](allenai.org/blog/paper-finder) tool).
